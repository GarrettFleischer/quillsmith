import { getNovelTree, listKnowledge, listOverviewAnswers } from "@/lib/novels";
...
export async function POST(
  ...
  // Replace structure into current novel id
  const {
    deleteAct,
    getNovelTree,
    upsertAct,
  ...
  for (const act of data.acts ?? []) {
    ...
    for (const chapter of (act.chapters as Array<Record<string, unknown>>) ?? []) {
      ...
      // upsertChapter creates a default scene — replace content from import
